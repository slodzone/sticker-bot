...
import { sendMessage, createNewStickerSet, addStickerToSet, deleteStickerFromSet, setStickerSetTitle, getStickerSet } from "@/lib/telegram";
...
// In-memory state for multi-step flows (resets on cold start — fine for Vercel)
const awaitingTitle: Record<number, boolean> = {};
const awaitingSticker: Record<number, string> = {}; // userId → packId
const awaitingDeleteChoice: Record<number, string> = {}; // userId → packId
const awaitingRename: Record<number, string> = {}; // userId → packId
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (awaitingTitle[userId] && !text.startsWith("/")) {
      ...
      await sendMessage(chatId, `⏳ Creating your sticker pack "<b>${title}</b>"...\n\nSend me the <b>first sticker</b> image to add (PNG or WebP, max 512×512):`);
    ...
    if (text.startsWith("/addto_")) {
      ...
      if (!pack || pack.ownerId !== userId) {
        await sendMessage(chatId, "Pack not found.");
        return NextResponse.json({ ok: true });
      }
    ...
    // ── Received a photo/document (sticker image) ─────────────────────────────
    if (awaitingSticker[userId] && (msg.photo || msg.document || msg.sticker)) {
      ...
      if (ctx.isNew) {
        ...
        await sendMessage(chatId,
          ...
          `🔑 PIN: <b>${ctx.pin}</b>\n\n` +
          `Share the link + PIN with collaborators so they can edit together!\n\n` +
    ...
    if (text === "/deletesticker") {
      ...
      if (packs.length === 1) {
        ...
        await sendMessage(chatId, `Which sticker to delete from "<b>${pack.title}</b>"?\n\n${list || "No stickers yet."}`);
      } else {
    ...
    if (text === "/sharepack") {
      ...
      await sendMessage(chatId, `Share these links + PINs with collaborators:\n\n${list}`);
      return NextResponse.json({ ok: true });
    }
